...
def parse_line(line: bytes, full: bool = False) -> tuple:
    """Split a collapsed frame stack sample into its components.

    These are: the process ID, the thread ID, the list of frames and the
    metrics. If ``full`` is true then the metrics include time and memory
    deltas; otherwise it is a tuple with a single value (either a time or a
    memory delta).

    Args:
        line (bytes): a single sample generated by Austin.
        full (bool): whether the sample has the full set of metrics or not.
            Defaults to ``False``.

    Returns:
        (tuple) the tuple of process ID, thread ID, list of frames and tuple
            of metrics.
    """
    rest = line.decode().strip("\n")
    process = None
    frames = []
    ...
    try:
        thread, rest = rest.split(";", maxsplit=1)
        frames, *metrics = rest.rsplit(maxsplit=3 if full else 1)
        frames = frames.split(";")
    except ValueError:
